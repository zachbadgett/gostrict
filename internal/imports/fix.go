...
// a group number.
var importToGroup = []func(sections, localPrefix, importPath string) (num int, ok bool){
	func(sections, localPrefix, importPath string) (num int, ok bool) {
		if localPrefix == "" {
			return
		}
	...
	func(sections, _, importPath string) (num int, ok bool) {
...
func importGroup(sections, localPrefix, importPath string) int {
	for _, fn := range importToGroup {
		if n, ok := fn(sections, localPrefix, importPath); ok {
			return n
		}
	}
	if sections != "" {
		s := strings.Split(sections, ",")
		if i := indexOf(s, "stdlib"); i > -1 {
			return i
		}
		panic("invalid section")
	}
