...
var (
	...
	nonewline = flag.Bool("n", false, "enable removing extra newline from function declarations")
...
func init() {
	...
	flag.StringVar(&options.Sections, "sections", "", "")
...
func processFile(filename string, in io.Reader, out io.Writer, argType argumentType) error {
	...
	if *nonewline {
		...
		f, err := parser.ParseFile(fset, "", res, 0)
		if err != nil {
			return err
		}
		var off int
		for _, v := range f.Decls {
			...
			case *ast.FuncDecl:
				...
				_, _ = b.Seek(int64(pos-1), io.SeekStart)
				for {
					e, err := b.ReadByte()
					if err == io.EOF {
						break
					...
					if e == '"' || e == '`' {
						if inString && stringByte == e && prev != '\\' {
							inString = false
							stringByte = byte(0)
						} else if stringByte == byte(0) {
							inString = true
							stringByte = e
						}
					}
